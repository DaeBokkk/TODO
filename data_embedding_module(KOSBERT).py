...
def save_to_specific_table(documents: List[Document], embeddings: Embeddings):
    ...
    # 2. DB 연결 및 트랜잭션 처리
    conn = None
    BATCH_SIZE = 100  # 💡 [수정됨] 한 번에 밀어넣을 개수 제한
    ...
    try:
        ...
        # 💡 [수정됨] BATCH_SIZE(100개) 단위로 쪼개서 넣기
        for i in range(0, len(documents), BATCH_SIZE):
            ...
            # 💡 [수정됨] 100개 채울 때마다 DB에 확정(Commit) 도장 찍기
            conn.commit()
            success_count += len(batch_docs)
            print(f" -> 안전 적재 진행 중... ({success_count}/{len(documents)}건 완료)")
