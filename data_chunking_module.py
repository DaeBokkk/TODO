...
# ------------------------------------------------------------------------------
# 1000 토큰: 유료 임베딩 모델(OpenAI)의 성능을 활용하기 위해 청크 사이즈 확대
CHUNK_SIZE = 1000
# 150 토큰: 문맥 끊김 방지를 위해 중복 영역 확대
CHUNK_OVERLAP = 150
...
if __name__ == "__main__":
    # 파일 경로 (업로드한 파일명)
    file_path = '/Users/solseon/Desktop/TODO/txts/sm_real_estate/sm_rent_data_20260504.txt'
    ...
    if raw_data:
        ...
        print("\n--- [3단계] 결과 검증 (샘플 출력) ---")
        for i, chunk in enumerate(final_chunks[:4]): # 처음 2개만 출력
            print(f"\n[Chunk #{i+1}]")
